//document.getElementById("test").addEventListener("click", function () {alert(x)} );
...
app.controller('GetFuncListCtrl', function ($scope) {
    ...
    $scope.SysFunc = function () { bg.FuncHandler.GetFuncList((funcList) => { $scope.FuncList = funcList; $scope.$apply(); }, 0,true); }
    $scope.MyFunc = function () { bg.FuncHandler.GetFuncList((funcList) => { $scope.FuncList = funcList; $scope.$apply(); }, 0, false); }
